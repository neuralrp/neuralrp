...
Assistant:"""
    ...
    async def extract_character_scene_json_variation(
        ...
        Args:
            messages: All chat messages
            primary_character: Name of primary character to focus on
            primary_character_card: Character card data for context
            message_window: Number of recent messages to analyze (default: 20)
        ...
        # Construct VARIATION prompt - original structure, NO examples
        prompt = f"""Analyze the conversation and extract scene information as JSON.
...
Assistant:"""
        ...
        for attempt in range(3):
            if self.http_client is None or not self.config.get('kobold_url'):
                print("[SNAPSHOT VARIATION] LLM unavailable, skipping JSON extraction")
                return {}
            ...
            try:
                ...
                result = response.json().get('results', [{}])[0].get('text', '').strip()
                print(f"[SNAPSHOT VARIATION DEBUG] Attempt {attempt + 1}: '{result[:100]}...'")
                ...
                # Validate required fields
                if all(k in parsed for k in ['location', 'action', 'activity', 'dress', 'expression']):
                    print(f"[SNAPSHOT VARIATION] JSON extraction succeeded on attempt {attempt + 1}")
                    return parsed
                else:
                    print(f"[SNAPSHOT VARIATION] Attempt {attempt + 1}: Missing required fields: {list(parsed.keys())}")

            except json.JSONDecodeError as e:
                print(f"[SNAPSHOT VARIATION] Attempt {attempt + 1}: JSON parsing failed - {e}")
            except Exception as e:
                print(f"[SNAPSHOT VARIATION] Attempt {attempt + 1}: Error - {e}")

        print(f"[SNAPSHOT VARIATION] JSON extraction failed after 3 attempts")
        return {}
    ...
    async def analyze_scene_variation(self, messages: List[Dict], chat_id: str,
        ...
        """
        Simplified scene analysis using VARIATION mode for creative interpretations.
        ...
        Args:
            ...
            primary_character: Name of primary character to focus on
            primary_character_card: Character card data
        ...
        """
        # Use variation extraction with 20-message window
        scene_json = await self.extract_character_scene_json_variation(
            messages,
            primary_character,
            primary_character_card,
            message_window=20
        ...
        if not scene_json:
            print("[SNAPSHOT VARIATION] Extraction failed, returning empty scene")
